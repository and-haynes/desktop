...
| 5 | **Compact mode** | Done | Independent hide-sidebar and hide-toolbar settings. The initial chrome hides after the configured delay (3 s by default), leaving only the grabber. Tap or pull the grabber to reveal the toolbar; it stays until you tap or scroll the page. An explicitly opened sidebar stays until dismissed or a tab is chosen. Pulling the grabber only reveals the toolbar; gestures on the expanded bar follow its configured actions. |
