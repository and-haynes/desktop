...
struct ZenPalette: Equatable {
    ...
    init(accent: ZenColor, isDark: Bool) {
        ...
        let bg = isDark ? ZenTokens.brandingDark : ZenTokens.brandingPaper
        let bgReverse = isDark ? ZenTokens.brandingPaper : ZenTokens.brandingDark
        brandingBG = bg
        ...
        let tertiary =
            ...
            : accent.mix(.white, weight: 0.02)
        self.tertiary = tertiary

        // --zen-colors-hover-bg
        //   light: color-mix(in srgb, accent 90%, white 10%)
        //   dark:  color-mix(in srgb, accent 90%, branding-bg 10%)
        hoverBG = isDark ? accent.mix(bg, weight: 0.90) : accent.mix(.white, weight: 0.90)

        // --zen-colors-primary-foreground
        //   light: branding-bg-reverse
        //   dark:  color-mix(in srgb, accent 80%, white 20%)
        primaryForeground = isDark ? accent.mix(.white, weight: 0.80) : bgReverse
        ...
        dialogBackground = isDark ? ZenColor(hex: "#1c1c1c")! : ZenColor(hex: "#fafbff")!
        ...
        urlbarBackground =
            isDark
            ? accent.mix(ZenColor(24, 24, 24), weight: 0.04)
            : accent.mix(ZenColor(hex: "#f4f4f4")!, weight: 0.03)

        // --zen-main-browser-background: light-dark(rgb(235,235,235), #1b1b1b)
        mainBrowserBackground =
            isDark ? ZenColor(hex: "#1b1b1b")! : ZenColor(235, 235, 235)

        // --zen-themed-toolbar-bg-transparent: light-dark(branding-bg, #171717)
        themedToolbarBG = isDark ? ZenColor(hex: "#171717")! : bg

        // --zen-sidebar-notification-bg:
        //   color-mix(in srgb, accent 5%, light-dark(white, black))
        sidebarNotificationBG = accent.mix(isDark ? .black : .white, weight: 0.05)

        // --zen-toolbar-element-bg-hover:
        //   light-dark(rgba(0,0,0,.08), rgba(255,255,255,.1))
        toolbarElementHoverBG =
            isDark ? ZenColor(r: 1, g: 1, b: 1, a: 0.10) : ZenColor(r: 0, g: 0, b: 0, a: 0.08)
